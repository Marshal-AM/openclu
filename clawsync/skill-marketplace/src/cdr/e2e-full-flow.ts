 ...
 * Usage: npm run e2e
 *
 * Step 1 always upserts the full Catalog row (like Supabase). Uses manifest peer hints
...
config({ path: resolve(process.cwd(), "../../../../skill-capture/db/src/.env"), override: false });
...
async function stepCatalogRefresh(): Promise<void> {
  ...
  try {
    ...
    const { fetchSkillListingFromCatalog } = await import(
      "../../../../../skill-capture/db/src/src/lib/cdr-listing.js"
    );
...
async function stepcatalogQuery(manifestCid: string): Promise<void> {
  try {
    const { searchNaturalLanguage } = await import(
      "../../../../../skill-capture/db/src/src/services/query-catalog.js"
    );
