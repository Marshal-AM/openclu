...
export const query = action({
  ...
  handler: async (_ctx, args) => {
    ...
    return await runMarketplaceCli('query', body);
  },
});
...
export const getDetail = action({
  ...
  handler: async (_ctx, args) => {
    return await runMarketplaceCli('get-detail', args.skillName);
  },
});
...
export const stats = action({
  ...
  handler: async (_ctx, args) => {
    ...
    if (scope === 'mine') {
      const wallet = await runMarketplaceCli<{ address: string | null }>('wallet-address');
      if (!wallet.address) {
        throw new Error('AGENT_PRIVATE_KEY is required for "mine" stats scope.');
      }
      ownerAddress = wallet.address;
    }
    return await runMarketplaceCli('stats', { scope, ownerAddress });
  },
});
