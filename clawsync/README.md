...
PURCHASED_SKILLS_DIR=             # Optional; default data/purchased-skills
SKIP_LICENSE_MINT=                  # Dev only: reuse LICENSE_TOKEN_ID
LICENSE_TOKEN_ID=
```

`npm install` at the repo root also installs `skill-marketplace` (npm workspace + postinstall).

SyncBoard routes: **Purchase Agent Skills** (`/syncboard/skills/purchase`), **My Purchased Skills** (`/syncboard/skills/purchased`).

**Fresh-device buyers:** Purchases fetch encrypted bundles from **public IPFS** by catalog CID (Pinata pin at publish). Set `PINATA_API_KEY` + `PINATA_SECRET_KEY` in `.env` when distributing; buyers use `ops.ipfsGatewayUrl` or `PINATA_BUYER_GATEWAY` (default `https://gateway.pinata.cloud/ipfs`). Re-distribute or `npm run repin-pinata -- <skill>` for older listings.

### Manual test checklist (marketplace)

1. Set `AGENT_PRIVATE_KEY` and model keys in Convex env; run `cd skill-marketplace && npm install`.
2. `npx convex dev` and `npm run dev`.
3. SyncBoard → Purchase Agent Skills: search, browse catalog, open detail, purchase (wallet needs Aeneid IP).
4. My Purchased Skills → Import to agent → confirm skill on default agent Skills tab.
5. Chat: send a message that should invoke the imported knowledge skill.
6. SyncBoard → Models: select Groq and verify chat works with `GROQ_API_KEY`.
