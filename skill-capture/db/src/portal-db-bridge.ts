...
} from "./portal/mutate.js";
import type { PortalAvatar } from "./portal-types.js";
...
export async function portalUpsertUserAvatar(wallet: string, avatar: PortalAvatar) {
  ...
  return {
    avatarUrl: row.avatarUrl ?? `/api/profile/avatar/image?t=${Date.parse(row.updatedAt) || Date.now()}`,
  };
}
