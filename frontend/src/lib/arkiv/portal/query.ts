...
import { createPortalPublicClient, normalizeWalletAddress } from "./client";
...
function parseEntityPayload<T>(entity: Entity, schema: { parse: (v: unknown) => T }): T {
  return schema.parse(entity.toJson());
}

function toUserRow(entity: Entity): PortalUserRow {
  return { ...parseEntityPayload(entity, PortalUserPayloadSchema), entityKey: entity.key };
}

function toDeviceRow(entity: Entity): PortalDeviceRow {
  return { ...parseEntityPayload(entity, PortalDevicePayloadSchema), entityKey: entity.key };
}

function toPendingRow(entity: Entity): PendingRegistrationRow {
  return { ...parseEntityPayload(entity, PendingRegistrationPayloadSchema), entityKey: entity.key };
}
