...
import { ArkivError, wrapArkivError } from "@/lib/arkiv/errors";
import { createPortalWalletClient, normalizeWalletAddress } from "./client";
import {
  buildPendingRegistrationCreate,
  buildPendingRegistrationUpdate,
  buildPortalDeviceCreate,
  buildPortalDeviceUpdate,
  buildPortalUserCreate,
  buildPortalUserUpdate,
} from "./entities";
import { pendingRegistrationExpiresIn } from "./expiration";
import type {
...
export async function upsertPortalUser(input: UpsertPortalUserInput): Promise<PortalUserRow> {
  ...
  try {
    if (existing) {
      await walletClient.updateEntity(buildPortalUserUpdate(existing.entityKey as Hex, payload));
...
export async function updatePortalDeviceForOwner(
  ...
  const walletClient = createPortalWalletClient();
  try {
    await walletClient.updateEntity(buildPortalDeviceUpdate(existing.entityKey as Hex, payload));
    return { ...payload, entityKey: existing.entityKey };
  } catch (err) {
    throw wrapArkivError(err);
  }
