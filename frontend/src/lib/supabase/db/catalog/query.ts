...
import { getSupabase, normalizeWalletAddress } from "../client";
import { DbError } from "../errors";
import { LISTING_STATUS, type ListingStatus } from "../constants";
import {
...
function applyWalletFilter<T extends { eq: (column: string, value: string) => T }>(
  q: T,
  ...
  if (scope === "mine") {
    if (!filters.ownerAddress) {
      throw new DbError("VALIDATION_FAILED", "ownerAddress is required for scope=mine");
    }
...
export async function getCatalogStats(
  ...
  const tagQ = getSupabase()
    .from("catalog_listing_tags")
    .select("id", { count: "exact", head: true });
  const { count: skillTag } = await tagQ;

  const verQ = getSupabase()
    .from("catalog_listing_versions")
    .select("id", { count: "exact", head: true });
